...
def generate_noisy_function(function_as_np_expression, noise_loc, noise_scale, num_of_points, range_high, range_low):
    """
    Returns two arrays that represents a a set of noisy 2D points for a given function
        ...
        :param range_high: The upper range for the function generated
        :param range_low: The lower range for the function generated
    """
    noise = np.random.normal(noise_loc, noise_scale, num_of_points)

    sample_x = np.random.random(num_of_points) * range_high + range_low
